...
class Finance(View, LoginRequiredMixin):
    ...
    def get(self, request):
        ...
        events = Booking.objects.filter(
            start_time__year=datetime.now().year, user=self.request.user
        )
        ...
        months = [
            datetime(2000, x["start_time__month"], 1).strftime("%B")
            for x in group_income_expenses
        ]  # for each month convert datetime to the month
        ...
        ax.bar(
            [x - 0.2 for x in range(len(group_income_expenses))],
            income_per_month,
            width=0.4,
            label="Income",
        )
        ...
        ax.set_xticks(
            range(len(group_income_expenses))
        )  # get as many x-axes ticks as there are months to display
        ...
        return render(
            request,
            "finance.html",
            {"finance_bar_graph": income_expense_graph, "current_year": datetime.now()},
        )
