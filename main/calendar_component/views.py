...
def get_date(
    current_year_month,
):  # gets the current date and splits it out into year and month that can be used for the previous_month and next_month functions
...
def previous_month(month_navigation):
    first_day_in_month = month_navigation.replace(
        day=1
    )  # makes a new date object for the 1st day of the month
    previous_month = first_day_in_month - timedelta(
        days=1
    )  # -1 from 1st day of the month to get the last_day_in_month day of the previous month
    month = "month=" + str(previous_month.year) + "-" + str(previous_month.month)
    return month


def next_month(month_navigation):
    days_in_month = calendar.monthrange(month_navigation.year, month_navigation.month)[
        1
    ]  # get the days in the month
    last_day_in_month = month_navigation.replace(
        day=days_in_month
    )  # makes a new date object for the last day of the month
    next_month = last_day_in_month + timedelta(
        days=1
    )  # adds a day to the last day of the month to get the next month
...
def booking(request, booking_id=None):
    if (
        booking_id
    ):  # checks if there is a bookingid and if there is make sure it is for the user
    ...
    if (
        request.method == "POST"
    ):  # makes sure the booking form is sent using a POST request
        form = BookingForm(
            request.POST, instance=booking
        )  # adds booking details to the form
        if form.is_valid():
        ...
        form = BookingForm(
            instance=booking
        )  # if not a POST request form remains and booking is not saved
    return render(
        request, "booking.html", {"form": form}
    )  # displays booking page with the booking form
